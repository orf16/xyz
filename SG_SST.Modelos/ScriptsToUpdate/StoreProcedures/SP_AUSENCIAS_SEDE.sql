...
ALTER PROCEDURE [dbo].[SP_AUSENCIAS_SEDE] (@anio int, @idOrigen int, @idEmpresaUsuaria int, @idSede int, @idDepartamento int, @nitEmpresa varchar(20))
AS
BEGIN
   ...
   declare @condicion2 VARCHAR(1000) = ' order by sd.Nombre_Sede, a.fechainicio';
   declare @consulta VARCHAR(4000) = 'select c.PK_Id_Contingencia as idContigencia, sd.Nombre_Sede as Descripcion, a.fechainicio, a.Fecha_Fin as FechaFin 
									  from Tbl_Ausencias a 
									  inner join Tbl_Contingencias c on a.FK_Id_Contingencia = c.PK_Id_Contingencia
									  inner join Tbl_Sede sd on a.FK_Id_Sede = sd.Pk_Id_Sede';
