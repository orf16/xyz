ALTER PROCEDURE [dbo].[SP_AUSENCIAS_CANTIDAD_ENFERMEDADES] (@anio int, @idOrigen int, @idEmpresaUsuaria int, @idSede int, @idDepartamento int, @nitEmpresa varchar(20))
AS
BEGIN
   ...
   declare @condicion2 VARCHAR(1000) = ' and a.FK_Id_Ausencias_Padre = 0 group by d.PK_Id_Diagnostico, d.Capitulo, DATEPART(MONTH,a.fechainicio)';
   declare @consulta VARCHAR(4000) = 'select '''''''' Contingencia, d.Capitulo as Evento, 
									  DATEPART(MONTH,a.fechainicio) as Mes, cast(count(d.PK_Id_Diagnostico) AS DECIMAL(19,2)) as Total 
