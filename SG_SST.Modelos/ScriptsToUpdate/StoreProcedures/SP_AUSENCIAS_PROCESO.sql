...
ALTER PROCEDURE [dbo].[SP_AUSENCIAS_PROCESO] (@anio int, @idOrigen int, @idEmpresaUsuaria int, @idSede int, @idDepartamento int, @nitEmpresa varchar(20))
AS
BEGIN
   ...
   declare @condicion2 VARCHAR(1000) = ' order by p.Descripcion_Proceso, a.fechainicio';
   declare @consulta VARCHAR(4000) = 'select c.PK_Id_Contingencia as idContigencia, p.Descripcion_Proceso as Descripcion, a.fechainicio, a.Fecha_Fin as FechaFin 
									  from Tbl_Ausencias a 
									  inner join Tbl_Contingencias c on a.FK_Id_Contingencia = c.PK_Id_Contingencia
									  inner join Tbl_Proceso p on a.FK_Id_Proceso = p.Pk_Id_Proceso';
