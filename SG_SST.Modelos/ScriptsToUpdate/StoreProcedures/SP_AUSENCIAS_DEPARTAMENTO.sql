...
ALTER PROCEDURE [dbo].[SP_AUSENCIAS_DEPARTAMENTO] (@anio int, @idOrigen int, @idEmpresaUsuaria int, @idSede int, @idDepartamento int, @nitEmpresa varchar(20))
AS
BEGIN
   ...
   declare @condicion2 VARCHAR(1000) = ' order by d.Nombre_Departamento, a.fechainicio';
   declare @consulta VARCHAR(4000) = 'select c.PK_Id_Contingencia as idContigencia, d.Nombre_Departamento as Descripcion, a.fechainicio, a.Fecha_Fin as FechaFin 
									  from Tbl_Ausencias a 
									  inner join Tbl_Contingencias c on a.FK_Id_Contingencia = c.PK_Id_Contingencia
									  inner join Tbl_Departamento d on a.FK_Id_Departamento = d.Pk_Id_Departamento';
