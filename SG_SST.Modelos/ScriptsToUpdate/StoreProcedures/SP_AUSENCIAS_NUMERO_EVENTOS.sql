...
ALTER PROCEDURE [dbo].[SP_AUSENCIAS_NUMERO_EVENTOS] (@anio int, @idOrigen int, @idEmpresaUsuaria int, @idSede int, @idDepartamento int, @nitEmpresa varchar(20))
AS
BEGIN
   ...
   declare @condicion2 VARCHAR(1000) = ' and a.FK_Id_Ausencias_Padre = 0 group by c.Detalle, DATEPART(MONTH,a.fechainicio)';
   declare @consulta VARCHAR(4000) = 'select c.Detalle as Contingencia, '''''''' as Evento, 
