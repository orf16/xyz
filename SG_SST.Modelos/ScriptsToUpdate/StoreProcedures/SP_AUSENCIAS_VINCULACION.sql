...
ALTER PROCEDURE [dbo].[SP_AUSENCIAS_VINCULACION] (@anio int, @idOrigen int, @idEmpresaUsuaria int, @idSede int, @idDepartamento int, @nitEmpresa varchar(20))
AS
BEGIN
   ...
   declare @condicion2 VARCHAR(1000) = ' and a.FK_Id_Ausencias_Padre = 0 group by a.Tipo_Vinculacion, DATEPART(MONTH,a.fechainicio)';
   declare @consulta VARCHAR(4000) = 'select '''''''' as Contingencia, case a.Tipo_Vinculacion when ''''NULL'''' then ''''No Especificada'''' when null then ''''No Especificada'''' else a.Tipo_Vinculacion end as Evento, 
